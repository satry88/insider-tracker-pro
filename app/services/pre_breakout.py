...
def pre_breakout_score(ohlcv, benchmark_closes=None):
    ...
    return {
        ...
        "trade_plan": _trade_plan(ohlcv, comp["atr"].get("atr")),
    }


def _trade_plan(ohlcv, atr):
    """مستويات إدارة المخاطر بناءً على التذبذب (ATR) — تعليمية لا توصية.
    وقف الخسارة = الدخول − 2×ATR ، والأهداف بنِسَب مخاطرة/عائد 2:1 و3:1."""
    price = ohlcv[-1]["close"]
    if not atr or atr <= 0 or not price:
        return None
    risk = 2 * atr                       # مسافة المخاطرة (وقف الخسارة)
    return {
        "entry":       round(price, 2),
        "stop":        round(price - risk, 2),
        "stop_pct":    round(-risk / price * 100, 1),
        "target1":     round(price + 2 * risk, 2),
        "target1_pct": round(2 * risk / price * 100, 1),
        "target2":     round(price + 3 * risk, 2),
        "target2_pct": round(3 * risk / price * 100, 1),
        "atr":         round(atr, 2),
        "rr1": "2:1", "rr2": "3:1",
    }
